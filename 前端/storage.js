...
function markDirty() { if (!isDirty) { saveStatus.textContent = "有未保存改动"; statusDot.classList.add("is-dirty"); } isDirty = true; if (typeof updateTruthStatus === "function") updateTruthStatus(); clearTimeout(autosaveTimer); autosaveTimer = setTimeout(() => { autosaveTimer = null; if (saveLocal._backoff) return; saveLocal("已自动保存"); }, AUTOSAVE_DELAY_MS); }
