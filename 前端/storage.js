...
function saveLocal(message = "已保存到磁盘") { clearTimeout(autosaveTimer); autosaveTimer = null; try { const nextRev = Math.max(stateRev, lastSeenRemoteRev) + 1; localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...state, rev: nextRev, updatedAt: new Date().toISOString() })); stateRev = nextRev; saveStatus.textContent = message; statusDot.classList.remove("is-dirty"); isDirty = false; saveLocal._warned = false; saveLocal._backoff = false; if (typeof updateTruthStatus === "function") updateTruthStatus(); persistToDiskSilent(); return true; } catch { saveStatus.textContent = message === "已自动保存" ? "自动保存失败" : "保存失败"; statusDot.classList.add("is-dirty"); isDirty = true; saveLocal._backoff = true; if (typeof updateTruthStatus === "function") updateTruthStatus(); if (!saveLocal._warned) { saveLocal._warned = true; showToast("本地存储空间不足，请立即导出 JSON 备份"); } return false; } }

function markDirty() { if (typeof cancelAutoReloadFromDisk === "function") cancelAutoReloadFromDisk(); if (!isDirty) { saveStatus.textContent = "有未保存改动"; statusDot.classList.add("is-dirty"); } isDirty = true; if (typeof updateTruthStatus === "function") updateTruthStatus(); clearTimeout(autosaveTimer); autosaveTimer = setTimeout(() => { autosaveTimer = null; if (saveLocal._backoff) return; saveLocal("已自动保存"); }, AUTOSAVE_DELAY_MS); }
